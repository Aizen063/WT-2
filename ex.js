...
app.get('/task3', (req, res) => {
    const result = spiData.filter(item => item.spi > 5);
    res.send(result);
  });
  
 
  app.get('/task4', (req, res) => {
    const count = spiData.filter(item => item.spi < 5).length;
    res.send({ count });
  });
  
  
  app.get('/task5', (req, res) => {
    const result = spiData.filter(item => item.language === 'Hindi');
    res.send(result);
  });
  
 
  app.get('/task6', (req, res) => {
    const result = spiData.filter(
      item => item.language === 'Sindhi' && item.name.startsWith('A')
    );
    res.send(result);
  });
